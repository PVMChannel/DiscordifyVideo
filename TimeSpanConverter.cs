...
public class TimeSpanConverter : IValueConverter
{
    ...
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        if (value is TimeSpan timeSpan)
        {
            ...
            {
                return $"{timeSpan.Hours.ToString().PadLeft(2, '0')}:{timeSpan.Minutes.ToString().PadLeft(2, '0')}:{timeSpan.Seconds.ToString().PadLeft(2, '0')}{culture.NumberFormat.NumberDecimalSeparator}{Math.Round((double) timeSpan.Milliseconds / 10).ToString().PadLeft(2, '0')}";
            }
        }
    ...
    public object ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        if(value is string timeString)
        {
            try
            {
                ...
                return new TimeSpan(0, timeStringSplitByColon[0], timeStringSplitByColon[1], timeStringSplitByColon[2], Int32.Parse(timeStringSplitByDot[1].PadRight(3, '0')));
            }
            catch (Exception)
            {
